...
(function () {
    ...
    if (isIframe) {
      window.addEventListener("message", function (event) {
        ...
        if (dataParsed.eventType == "set_custom_style") {
          ...
          dataParsed.eventType == "reload_iframe"
        ) {
          try {
  ...
  function openProtoUrl(url) {
    ...
    if (useIframe) {
      var iframeContEl =
        document.getElementById("tgme_frame_cont") ||
        document.body;
...
(function () {
  ...
  if (
    ...
    initParams.tgWebAppData.length
  ) {
    webAppInitData = initParams.tgWebAppData;
    webAppInitDataUnsafe =
      Utils.urlParseQueryString(webAppInitData);
  ...
  var stored_theme_params =
    Utils.sessionStorageGet("themeParams");
  if (
  ...
  var stored_def_colors =
    Utils.sessionStorageGet("defaultColors");
  if (
  ...
  var stored_fullscreen =
    Utils.sessionStorageGet("isFullscreen");
  ...
  function setSafeAreaInset(data) {
    ...
    setCssProperty(
      "safe-area-inset-top",
      safeAreaInset.top + "px"
    );
    setCssProperty(
      "safe-area-inset-bottom",
      safeAreaInset.bottom + "px"
    );
    setCssProperty(
      "safe-area-inset-left",
      safeAreaInset.left + "px"
    );
    setCssProperty(
      "safe-area-inset-right",
      safeAreaInset.right + "px"
    );
  }
  ...
  function onFullscreenFailed(eventType, eventData) {
    if (
      eventData.error == "ALREADY_FULLSCREEN" &&
      !webAppIsFullscreen
    ) {
      setFullscreen(true);
    }
  ...
  function onEmojiStatusAccessRequested(
    eventType,
    eventData
  ) {
  ...
  function parseColorToHex(color) {
    ...
    if ((match = /^\s*#([0-9a-f]{6})\s*$/i.exec(color))) {
      ...
      (match =
        /^\s*#([0-9a-f])([0-9a-f])([0-9a-f])\s*$/i.exec(
          color
        ))
    ) {
      return (
  ...
  if (initParams.tgWebAppDebug) {
    debugBottomBar =
      document.createElement("tg-bottom-bar");
  ...
  var BottomButtonConstructor = function (type) {
    ...
    function updateDebugButton(btn_params) {
      if (btn_params.is_visible) {
        ...
        debugBtn.style.opacity = btn_params.is_active
          ? "1"
          : "0.8";
        ...
        debugBtn.className = btn_params.has_shine_effect
          ? "shine"
          : "";
        debugBtn.style.backgroundImage =
          btn_params.is_progress_visible
    ...
    function setParams(params) {
      if (
        typeof params.icon_custom_emoji_id !== "undefined"
      ) {
      ...
      if (typeof params.text_color !== "undefined") {
        if (
          ...
          var text_color = parseColorToHex(
            params.text_color
          );
          if (!text_color) {
            console.error(
  ...
  var SecondaryButton =
    BottomButtonConstructor("secondary");
  ...
  var DeviceStorage = (function () {
    ...
    function onDeviceStorageEvent(eventType, eventData) {
      if (
        eventData.req_id &&
        webAppCallbacks[eventData.req_id]
      ) {
        ...
        if (eventType == "device_storage_failed") {
          ...
          eventType == "device_storage_key_received"
        ) {
          res = eventData.value;
        } else {
          res = true;
    ...
    function onSecureStorageEvent(eventType, eventData) {
      if (
        ...
        if (eventType == "secure_storage_failed") {
          ...
          eventType == "secure_storage_key_received"
        ) {
          res = eventData.value;
          ...
          eventType == "secure_storage_key_restored"
        ) {
          res = eventData.value;
        } else {
          res = true;
  ...
  var BiometricManager = (function () {
    ...
    biometricManager.requestAccess = function (
      params,
      callback
    ) {
      if (!checkVersion()) {
    ...
    biometricManager.authenticate = function (
      params,
      callback
    ) {
      if (!checkVersion()) {
    ...
    biometricManager.updateBiometricToken = function (
      token,
      callback
    ) {
      if (!checkVersion()) {
  ...
  var LocationManager = (function () {
    ...
    function onLocationRequested(eventType, eventData) {
      if (!eventData.available) {
        ...
        if (
          typeof eventData.horizontal_accuracy !==
            "undefined" &&
        ...
        if (
          typeof eventData.vertical_accuracy !==
            "undefined" &&
        ...
        if (
          typeof eventData.course_accuracy !==
            "undefined" &&
          eventData.course_accuracy !== null
        ) {
          locationData.course_accuracy =
            eventData.course_accuracy;
        }
        if (
          typeof eventData.speed_accuracy !== "undefined" &&
          eventData.speed_accuracy !== null
        ) {
          locationData.speed_accuracy =
            eventData.speed_accuracy;
        }
      }
      if (
        !eventData.available ||
    ...
    function locationResponse(response) {
      if (getRequestState.callbacks.length > 0) {
        for (
          var i = 0;
          i < getRequestState.callbacks.length;
          i++
        ) {
    ...
    locationManager.getLocation = function (callback) {
      ...
      if (!isLocationAvailable) {
        ...
        throw Error(
          "WebAppLocationManagerLocationNotAvailable"
        );
      }
    ...
    locationManager.openSettings = function () {
      ...
      if (!isAccessRequested) {
        ...
        throw Error(
          "WebAppLocationManagerLocationAccessNotRequested"
        );
      }
      if (isAccessGranted) {
        console.warn(
  ...
  var Accelerometer = (function () {
    ...
    accelerometer.start = function (params, callback) {
      ...
      var req_params = {};
      var refresh_rate = parseInt(
        params.refresh_rate || 1000
      );
      if (
        isNaN(refresh_rate) ||
        refresh_rate < 20 ||
  ...
  var DeviceOrientation = (function () {
    ...
    function onDeviceOrientationStopped(
      eventType,
      eventData
    ) {
      isStarted = false;
    ...
    function onDeviceOrientationChanged(
      eventType,
      eventData
    ) {
    ...
    function onDeviceOrientationFailed(
      eventType,
      eventData
    ) {
      if (startCallbacks.length > 0) {
  ...
  WebApp.switchInlineQuery = function (
    query,
    choose_chat_types
  ) {
    ...
    WebView.postEvent(
      ...
      false,
      { query: query, chat_types: chat_types }
    );
  };
  ...
  WebApp.downloadFile = function (params, callback) {
    ...
    if (
      !params ||
      !params.file_name ||
      !params.file_name.length
    ) {
      console.error(
        "[Telegram.WebApp] File name is required"
      );
      throw Error("WebAppDownloadFileParamInvalid");
    }
  ...
  WebApp.shareToStory = function (media_url, params) {
    ...
    if (typeof params.text !== "undefined") {
      ...
      if (text.length > 2048) {
        console.error(
          "[Telegram.WebApp] Text is too long",
          text
        );
  ...
  WebApp.shareMessage = function (msg_id, callback) {
    ...
    if (WebAppShareMessageOpened) {
      console.error(
        "[Telegram.WebApp] Share message is already opened"
      );
  ...
  WebApp.requestChat = function (req_id, callback) {
    ...
    if (WebAppRequestChatOpened) {
      console.error(
        "[Telegram.WebApp] Request chat is already opened"
      );
  ...
  WebApp.setEmojiStatus = function (
    custom_emoji_id,
    params,
    callback
  ) {
  ...
  WebApp.invokeCustomMethod = function (
    method,
    params,
    callback
  ) {
  ...
  WebView.onEvent(
    ...
    onPreparedMessageSent
  );
  WebView.onEvent(
    "prepared_message_failed",
    onPreparedMessageFailed
  );
  WebView.onEvent(
    "requested_chat_sent",
    onRequestedChatSent
